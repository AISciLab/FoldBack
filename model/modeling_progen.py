...
from .structure import Seq3DAttention
...
class CrossAttention(nn.Module):
    def __init__(self, config):
        ...
        self.esm_dim = config.cross_dim

        self.query_proj = nn.Linear(self.embed_dim,self.embed_dim,bias=False).float()
        self.key_proj = nn.Linear(self.esm_dim,self.embed_dim,bias=False).float()
        self.value_proj = nn.Linear(self.esm_dim,self.embed_dim,bias=False).float()
        self.cross_out = nn.Linear(self.embed_dim,self.embed_dim,bias=False).float()

        self.num_attention_heads = config.cross_head
    ...
    def forward(
        ...
        mp_num = self.num_attention_heads
        local_dim = self.head_dim
        query = self.query_proj(hidden_state)
        query_split = query.reshape(query.shape[:-1] + (mp_num, -1))
        key = self.key_proj(esm_memory)
        key_split = key.reshape(key.shape[:-1] + (mp_num, -1))
        value = self.value_proj(esm_memory)
        value_split = value.reshape(value.shape[:-1] + (mp_num, -1))

        value_split = value_split.permute(0, 2, 1, 3)
        key_split = key_split.permute(0, 2, 1, 3)
        query_split = query_split.permute(0, 2, 1, 3)

        attn_output, attn_weights = self._attn(query_split, key_split, value_split, cross_mask)
        attn_output = self._merge_heads(attn_output, mp_num, local_dim)
        attn_output = self.cross_out(attn_output)
...
class ProGenBlock1(nn.Module):
    ...
    def forward(
        ...
        struct_emb = self.struct(seq_emb,struct_emb,struct_mask)

        cross_outputs = self.cross(
            hidden_state=hidden_states,
            esm_memory = struct_emb,
            cross_mask = cross_mask
        )
        tmp_hidden_states = cross_outputs
        ln_hidden_states = self.ln_2(tmp_hidden_states)
        feed_forward_states = self.mlp(ln_hidden_states)
        hidden_states = feed_forward_states+hidden_states
        if use_cache:
...
class ProGenModel(ProGenPreTrainedModel):
    def __init__(self, config):
        ...
        self.h = nn.ModuleList(
            [ProGenBlock1(config) if i % 3 != 0 else ProGenBlock2(config) for i in range(config.n_layer)]
        )
    ...
    def forward(
        ...
        if receptor_mask is not None:
            assert batch_size > 0, "batch_size has to be defined and > 0"
            cross_mask = receptor_mask.view(batch_size, -1)
            cross_mask = cross_mask[:, None, None, :]
            cross_mask = cross_mask.to(dtype=self.dtype)
            cross_mask = (1.0 - cross_mask) * -10000.0
            if struct_emb is not None:
                struct_mask = torch.tile(receptor_mask,(1,2))
                cross_mask = torch.tile(cross_mask, (1, 1, 1, 2))
...
class ProGenForCausalLM(ProGenPreTrainedModel):
    ...
    def prepare_inputs_for_generation(self, input_ids, past=None, **kwargs):
        ...
        seq_emb = kwargs.get("seq_emb", None)
        struct_emb = kwargs.get("struct_emb", None)
        receptor_mask = kwargs.get("receptor_mask", None)
    ...
    def forward(
        ...
        labels (:obj:`torch.LongTensor` of shape :obj:`(batch_size, sequence_length)`, `optional`):
            Labels for language modeling. Note that the labels **are shifted** inside the model1, i.e. you can set
